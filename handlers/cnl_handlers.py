    ...
    if data == "cnl:gcopy" or data.startswith("cnl:gcopy:"):
        ...
        if action == "tog":
            if gc.get("enabled"):
                ...
                # Drop Global Copy dependency so account client can stop if unused
                try:
                    aid = gc.get("my_account_id")
                    if aid:
                        from core.lifecycle import release_my_account
                        await release_my_account(user_id, str(aid), "cnl:gcopy")
                except Exception:
                    logger.exception("gcopy disable release account user=%s", user_id)
            else:
                ...
                try:
                    ...
                    ok, msg = await acquire_my_account(
                        user_id, str(gc["my_account_id"]), "cnl:gcopy"
                    )
                    if not ok:
                        logger.warning(
                            "gcopy enable: account start failed user=%s: %s",
                            user_id, msg,
                        )
                        await query.answer(
                            f"Enabled but account client failed: {msg}",
                            show_alert=True,
                        )
                except Exception:
                    logger.exception("gcopy enable acquire account user=%s", user_id)
            gc = await cnl.get_global_copy(user_id) or {}
